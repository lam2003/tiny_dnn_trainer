        ...
        for (int i = 0; i < cplate_vec.size(); i++)
        {
            ...
            if (mser_cchar_vec.size() < kPlateMaxCharNum)
            {
                ...
                if (!is_chinese)
                {
                    ...
                    slideWindowSearch(scale_gray_mat, line_vec4f, left_point, right_point, max_cchar_rect, cplate_rect, left_slide_cchar_vec, otsu_level, 0.4, 0.8, true, LEFT);
                ...
                if (mser_cchar_vec.size() < kPlateMaxCharNum)
                {
                    slideWindowSearch(scale_gray_mat, line_vec4f, left_point, right_point, max_cchar_rect, cplate_rect, right_slide_cchar_vec, otsu_level, 0.4, 0.8, false, RIGHT);
            ...
            for (int j = 0; j < left_slide_cchar_vec.size(); j++)
            {
                cplate.addMserCChar(left_slide_cchar_vec[j]);
                if (display_process)
                    rectangle(scale_rgb_mat, left_slide_cchar_vec[j].getRect(), Scalar(0, 0, 255), 1);
            }

            for (int j = 0; j < right_slide_cchar_vec.size(); j++)
            {
                cplate.addMserCChar(right_slide_cchar_vec[j]);
                if (display_process)
                    rectangle(scale_rgb_mat, right_slide_cchar_vec[j].getRect(), Scalar(255, 0, 0), 1);
            }

            if (display_process && color_index == cplate.getColor())
            rectangle(scale_rgb_mat, cplate_rect, Scalar(255, 0, 255), 2);
